...
void idDoor::Save( idSaveGame *savefile ) const {
	...
	savefile->WriteString( strRequires );
...
void idDoor::Restore( idRestoreGame *savefile ) {
	...
	savefile->ReadString( strRequires );
...
void idDoor::Spawn() {
	...
	spawnArgs.GetString( "requires", "", strRequires );
...
void idDoor::Use( idEntity *other, idEntity *activator ) {
	if ( gameLocal.RequirementMet( activator, strRequires, removeItem ) ) {
		if ( syncLock.Length() ) {
